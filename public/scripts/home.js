...
function saveGame(event){
    ...
    fetch(BASE_URL + "save-game", {method: 'POST', body: form_data}).then(dispatchResponse);
...
async function onJsonResponse(json){
    ...
    for(let game in json){
        ...
        console.log(id);
        fetch(BASE_URL+ "show/" + encodeURIComponent(id))
        .then((res) => {
            return res.json();
        })
        .then((game_json) => {
            ...
            if (typeof game_json.images.banner !== 'undefined') {
...
function search(event){
    ...
    fetch( BASE_URL+ "search/"+encodeURIComponent(form_data.get('search'))).then(onResponse).then(onJsonResponse)
}
